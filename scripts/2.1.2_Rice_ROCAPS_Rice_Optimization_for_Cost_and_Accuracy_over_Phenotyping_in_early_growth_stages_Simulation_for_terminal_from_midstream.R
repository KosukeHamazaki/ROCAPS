######################################################################################################################################################
###### 2.1.2_Rice_ROCAPS_Rice_Optimization_for_Cost_and_Accuracy_over_Phenotyping_in_early_growth_stages_Simulation_for_terminal_from_midstream ######
######################################################################################################################################################
...
scriptID <- 2.1
trial.no.pred <- 1
target.traits <- c("Sim", commandArgs(trailingOnly = T))
method <- "MTM"
...
for(trait.no in 1:10){
  ...
  dirExists <- dir.exists(dir.mid.ROCAPS)

  if (dirExists) {
    listFiles <- list.files(dir.mid.ROCAPS)
    countFin <- max(as.numeric(listFiles), na.rm = TRUE) - 1
  } else {
  ...
  if (dirExists & (countFin > 0)) {
    convergenceCheck <- "midstream_all_info.Rdata" %in%
      list.files(paste0(dir.mid.ROCAPS, "/", countFin + 1))

    if (convergenceCheck) {
      fileMidAll <- paste0(dir.mid.ROCAPS, "/", countFin + 1,
                           "/midstream_all_info.Rdata")
    } else {
      fileMidAll <- paste0(dir.mid.ROCAPS, "/", countFin,
                           "/midstream_all_info.Rdata")
    }
